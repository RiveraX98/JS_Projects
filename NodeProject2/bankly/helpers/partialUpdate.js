...
async function sqlForPartialUpdate(table, items, key, id) {
  ...
  for (let key in items) {
    ...
    //fix bug#5
    key === "password"
      ? (items[key] = await bcrypt.hash(items[key], BCRYPT_WORK_FACTOR))
      : key;
  }
  ...
  let query = `UPDATE ${table} SET ${cols} WHERE ${key}=$${idx} RETURNING username,first_name,last_name,email,phone`;
